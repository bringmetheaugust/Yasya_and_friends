...
export default class Game extends React.Component {
	...
	checkAttack() {
		...
		this.checkAttack = setInterval(() => {
			...
			for(let i of divs) {
				if (
					((cl > i.getBoundingClientRect().left && cl < i.getBoundingClientRect().right) ||
						(cr > i.getBoundingClientRect().left && cr < i.getBoundingClientRect().right)) &&
					((ct > i.getBoundingClientRect().top && ct < i.getBoundingClientRect().bottom) ||
						(cb < i.getBoundingClientRect().bottom && cb > i.getBoundingClientRect().top))
				) {
		...
		return(
			<section onClick={this.heroMove} className='game'>
				...
				<HeroContext.Consumer>
					{val => (
						<React.Fragment>
							<div id='hero'><Img className='hero-photo' img={val.photo}/></div>
							...
							<div className='attack'>
								{
									this.state.attackMassive.map((i,n) =>
										(<div style={i.style} key={n} className={i.className}>
											<Img img={val.block}/>
										</div>)
									)
								}
				...
				{this.state.gameOver?null:<div className='points'>очки : {this.state.points}</div>}
			</section>
		)
	}
}